...
describe "Acts As Taggable On" do
  ...
  describe "@@remove_unused_tags" do
    before do
      ...
      @tagging = ActsAsTaggable::Extra::Tagging.create(:taggable => @taggable, :tag => @tag, :context => 'tags')
    end
    ...
    context "if set to true" do
      ...
      it "should remove unused tags after removing taggings" do
        @tagging.destroy
        ActsAsTaggable::Tag.find_by_name("awesome").should be_nil
      end
    end

    context "if set to false" do
      before do
        ActsAsTaggable.remove_unused_tags = false
      end

      it "should not remove unused tags after removing taggings" do
        @tagging.destroy
        ActsAsTaggable::Tag.find_by_name("awesome").should == @tag
      end
    end
  end
